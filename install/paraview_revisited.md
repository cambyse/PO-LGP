...
#   Or manually:
# * ccmake ..
# * Press [c]configure to build up the options
# * Set BUILD_SHARED_LIBS to ON
# * Set CMAKE_BUILD_TYPE to Release
# * Set BUILD_TESTING to OFF
# * Press [c]onfigure two times, and then [g]enerate (pay attention to the error
#   messages listed at the end of the [c]onfigure step, since some needed libraries
...
#   step
# * Since all ParaView 4.2 libraries have the "-pv4.2" suffix attached to their
#   names, some symbolic links have to be added:
cd ~/para/ParaView/Build/lib/
for i in *-pv*.so; do ln -s $i ${i%-pv*}.so; done

# This will build ParaView as well as all the submodules (such as VTK).
# Optionally, you can specify -jN, e.g., make -j4 to speed up the compilation
# process by using more cores.
#
...
# * You may want to add "Build" to .gitignore to avoid committing the whole build directory if you were to commit
cd Build
cmake -DVTK_DIR=~/para/ParaView/Build/VTK -DCMAKE_CXX_FLAGS='-L~/para/ParaView/Build/lib' ..
# or manually
# * ccmake ..
# * Press [c]configure to build up the options
...
# * You may want to add "Build" to .gitignore to avoid committing the whole build directory if you were to commit
cd Build
cmake -DBUILD_SHARED_LIBS=ON -DPCL_DIR=~/para/pcl/Build -DParaView_DIR=~/para/ParaView/Build ..
# Or manually:
# * ccmake ..
# * Press [c]configure to build up the options
# * Set BUILD_SHARED_LIBS to ON
...
# * [c]onfigure and [g]enerate (pay attention to the error messages listed at the
