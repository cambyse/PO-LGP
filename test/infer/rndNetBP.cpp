...
void randomPairNet(VariableList& vars,FactorList &facs,uint N,uint n,double connectivity){
  ...
  //-- generate RVs
  vars.resize(N);
  for(i=0;i<N;i++) vars(i) = new Variable(n, STRING("var("<<i<<")"));

  //-- generate coupling factors
  Factor *f;
  for(i=0;i<N;i++) for(j=i+1;j<N;j++){
    if(rnd.uni()<connectivity){
      f = new Factor(TUPLE(vars(i), vars(j)));
      //f -> setRandom();
...
void rndNetBPwithExcludeEchoMessages(uint N,uint n,double connectivity,uint order){
  ...
  //-- generate RVs
  VariableList V(N);
  for(i=0;i<N;i++) V(i) = new Variable(n, STRING("var("<<i<<")"));

  //-- generate coupling factors
  FactorList F(N,N);  F.memMove=true;  F.setZero();
  for(i=0;i<N;i++) for(j=i+1;j<N;j++){
    if(rnd.uni()<connectivity){
      F(i,j) = new Factor(TUPLE(V(i), V(j)));
      //F(i,j) -> setRandom();
  ...
  for(i=0;i<N;i++) for(j=i+1;j<N;j++) if(F(i,j)){
    M(i,j) = new Factor(TUPLE(V(j)));
    M(j,i) = new Factor(TUPLE(V(i)));
  }
  FactorList M1(N,N,N),M2(N,N,N,N);
  for(i=0;i<N;i++) for(j=i+1;j<N;j++) if(F(i,j)){
    for(l=0;l<N;l++){
      M1(i,j,l) = new Factor(TUPLE(V(j)));
      M1(j,i,l) = new Factor(TUPLE(V(i)));
    }
    for(l=0;l<N;l++) for(ll=l+1;ll<N;ll++){
      M2(i,j,l,ll) = new Factor(TUPLE(V(j)));
      M2(j,i,l,ll) = new Factor(TUPLE(V(i)));
      M2(i,j,ll,l) = M2(i,j,l,ll);
      M2(j,i,ll,l) = M2(j,i,l,ll);
    }
  }
  
  MT::Array<Factor> marg(N);
  uint t;
  for(t=0;t<10;t++){ //LOOP
    if(order==0){
    //marginals
    ...
    //all messages
      for(i=0;i<N;i++) for(j=0;j<N;j++) if(i!=j && F(i,j)){
        ...
        tensorProductMarginal(*M(i,j), *F(i,j), f, TUPLE(V(i)));
      }
    }else{
      //marginals
      for(i=0;i<N;i++){
      ...
      for(i=0;i<N;i++) for(j=0;j<N;j++) if(i!=j && F(i,j)){
        ...
        for(l=0;l<N;l++){
          if(i==l) M1(i,j,l)->setOne();
            ...
            for(k=0  ;k<N;k++) if(k!=j && k!=i && F(k,i)){
              ...
              else         tensorMultiply(f,*M2(k,i,l,i));
            }
            tensorProductMarginal(*M1(i,j,l), *F(i,j), f, TUPLE(V(i)));
        ...
        if(order==2) for(l=0;l<N;l++) for(ll=l+1;ll<N;ll++){
          if(l==ll) continue;
          if(i==l || i==ll) M2(i,j,l,ll)->setOne();
          else{ //collect at i
            f=*E(i);
            for(k=0  ;k<N;k++) if(k!=j && k!=i && F(k,i)) tensorMultiply(f,*M2(k,i,l,ll));
            tensorProductMarginal(*M2(i,j,l,ll), *F(i,j), f, TUPLE(V(i)));
