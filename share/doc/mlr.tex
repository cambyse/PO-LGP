...
\begin{itemize}
\item We have a set of activities (\textbf{acts})
\item Each act has an integer (discrete!) \textbf{status}. (While Access data
  is often continuous (arrays), with update revisions in high
  frequencies, the status of an act should correspond to a categorial
  change (change of state/fact in RAP), perhaps triggering a script to
  respond to this change.)
\item Example acts are: control tasks (CtrlTask), controller loops
  (TaskController), sensor triggers (listening to ROS topics or
  accesses and translating them to status signals), computational
  threads (kinect processing, path planning), visualization threads
  (KinViewer, PathViewer), physical simulation threads (Physx),
  communication threads (ROS).
\item Each act may create/destroy other acts (Actor
  Model in Computer Science)
\item The core way to react on status signals is via the
  \textbf{wait}: It pauses the execution of a script until the status
  of some acts changes. After that the script queries the status and
  branches (if-then-else) based on that.
\item An act can be a \textbf{policy}, meaning that it is itself a
  script that creates/destroys other acts depending on status signals
  and data. That is: Often we just have a single serial main
  script. However, we can encapsulate such scripts to become policies
  and run them in parallel reactively. (This is in analogy to Rules in
  RAPs.)
\end{itemize}
...
//equivalently
wait(liftHand, R_done);

//idle wait for multiple activities conjunctive
waitAnd({liftHand, gazeBall});
//or
waitAnd({liftHand, gazeBall}, {R_done, R_done});

//idle wait for multiple activities disjunctive
waitOr({liftHand, gazeBall}, {R_done, R_done});
