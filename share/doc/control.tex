...
\item[Optimal acceleration law] We compute the optimal acceperation
  ...
  $\dot q$. For brevity of derivation we consider the null cost reference
  as another task law:
\begin{align}
\ddot y^*
...
\ddot q^*
 ...
 &= \bar K_p q + \bar K_d \dot q + \bar k  \label{eqProjGains}\\
\bar K_p
...
To explicitly include the null cost reference we replace
$$\mat{cc}{H & 0 \\ 0 & C} \gets C \comma \mat{c}{\Id \\ J} \gets
J ~.$$
The terms additively decompose as follows:
\begin{align}
\ddot q^*
 &= \argmin_{\ddot q} \norm{\ddot q - \ddot q_0^*}^2_H + \norm{J \ddot q - \ddot y^*}^2_C \\
 &\approx \bar K_p q + \bar K_d \dot q + \bar k  \label{eqProjGains}\\
\bar K_p
