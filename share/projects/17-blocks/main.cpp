...
void trial3(){
  ...
  //low-level implementation of distance, using GJK and inequality (>5cm)
//  komo.setTask(1.5,3., new TaskMap_GJK(komo.world, "yellow", "red", true, true), ineqTT, ARR(-.02), 1e2, 0);
  komo.setTask(1.5, 2.5, new TaskMap_Proxy(allPTMT, uintA(), .03), sumOfSqrTT, NoArr, 1e2);
...
void trial3b(){
  ...
  //-- fix last configuration
  komo.setTask(9.9, 10., new TaskMap_qItself(), sumOfSqrTT, Wfin.getJointState(), 1e3, 0);
  ...
  komo.setTask(3., 5., new TaskMap_Proxy(pairsPTMT, {W.getShapeByName("red")->index, W.getShapeByName("blue")->index}, .03), sumOfSqrTT, NoArr, 1e2);
...
void trial3c(){
  ...
  //-- fix last configuration
  komo.setTask(6.9, 7., new TaskMap_qItself(), sumOfSqrTT, Wfin.getJointState(), 1e3, 0);
