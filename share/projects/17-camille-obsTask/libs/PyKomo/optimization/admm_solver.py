...
class ADMMSolver:
    ...
    def to_0(f, xk, y, rho):
        class F(NewtonFunction):
            def value(self, x):
                delta = x - xk
                return f.value(x) + np.dot(y.T, delta) + 0.5 * rho * np.dot(delta.T, delta)

            def gradient(self, x):
                delta = x - xk
                return f.gradient(x) + y.T + rho * delta
    ...
    @staticmethod
    def to_1(f, x_fixed, y, rho):
        class F(NewtonFunction):
            def value(self, x):
                delta = x_fixed - x
                return f.value(x) + np.dot(y.T, delta) + 0.5 * rho * np.dot(delta.T,
                                                                                                  delta)
    ...
    def run(self, x):
        ...
        while True:
            ...
            f0 = self.to_0(self.pb.pb0.f, x1, self.y, self.rho)
            ...
            f1 = self.to_1(self.pb.pb1.f, x1, self.y, self.rho)
            pb1 = ConstrainedProblem(f=f1, h=self.pb.pb1.h, g=self.pb.pb1.g)
            assert f1.checkGradients(x1) and f1.checkHessian(x1)
