...
def test_build_circle():
    c = Circle(radius=2.0)
    nt.assert_equals(2.0, c.radius)

def test_draw_circle():
    c = Circle(radius=2.0)
    ax, patches = build_drawer()
    c.draw((0, 0, 0), patches)
    nt.assert_equals(1, len(patches))
    draw(ax, patches)
...
def test_add_shapes_kinematic_world():
    ...
    k.add_shape(Car())
    nt.assert_equals(2, len(k.shapes))
...
def test_create_joint():
    ...
    nt.assert_equals("circle", j.shape_name)

def test_add_joint_to_kin_engine():
    k = KinematicWorld()
    k.add_shape(Circle(name="circle", radius=2.0))
    k.add_joint(Joint(shape_name="circle"))
    nt.assert_equals(1, len(k.joints))

def test_add_joint_to_kin_engine():
    k = KinematicWorld()
    k.add_shape(Circle(name="circle", radius=2.0))
    k.add_joint(Joint(shape_name="circle"))
    nt.assert_equals(3, k.get_dim())

def test_set_pose_to_kin_engine():
    k = KinematicWorld()
    k.add_shape(Circle(name="circle", radius=2.0))
    k.set_fixed_pose("circle", (1, 0, 0))
    nt.assert_equals( (1, 0, 0), k.get_fixed_pose("circle") )
