...
joint (table1 redBall) { from=<T t(0 0 .02)> to=<T t(.3 -.0 .03)> type=JT_rigid }

### tools

body stick1 {}
joint (table1 stick1) { from=<T t(0 0 .02)> to=<T t(-.3 -.7 .02)> type=JT_rigid}
...
shape paperHandle(paper) { type=9 size=[.05 .05 .2 .02] color=[.6 .3 0] rel=<T t(-.4 0 .1)> contact }
