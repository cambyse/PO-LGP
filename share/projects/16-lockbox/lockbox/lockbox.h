...
//  bool moveToAlvar(const uint joint, const ors::Transformation& offset, const double speed = 1, const double prec = 10);
  ...
  bool moveAbsolute(const ors::Transformation& tf, const double speed = 1, const double prec = 10);
  bool moveToJoint(const uint joint, const ors::Transformation &offset_in_base, const double speed = 1, const double prec = 10);
  ...
  void moveJointInterpolate(const uint joint, const double position);

  //  std::unordered_map<uint, ors::Transformation> offsets = {

  std::unordered_map<uint, arr> offsets = {
    std::make_pair(1, ARR(-0.05, -0.13, 0.01)),
  ...
  std::unordered_map<uint, arr> end_offsets = {
    std::make_pair(1, ARR(-0.14, 0.34, 0.01)),
...
//private:
  ...
  std::unordered_map<uint, double> joint_angles = {
    std::make_pair(1, 0),
    std::make_pair(2, 0),
    std::make_pair(3, 0),
    std::make_pair(4, 0),
