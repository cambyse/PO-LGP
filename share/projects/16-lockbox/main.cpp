...
int main(int argc, char** argv){
  ...
  if (useRos)
  {
    ...
    // Alignment tasks.
    auto alignX = baxter.task("alignX", GRAPH(STRING("map=vecAlign ref1=endeffL ref2=alvar_10 vec1=[1 0 0] vec2=[0 0 -1] target=[1] prec=[1000]")));
    auto alignY = baxter.task("alignY", GRAPH(STRING("map=vecAlign ref1=endeffL ref2=alvar_10 vec1=[0 1 0] vec2=[1 0 0] target=[1] prec=[1000]")));
    auto alignZ = baxter.task("alignZ", GRAPH(STRING("map=vecAlign ref1=endeffL ref2=alvar_10 vec1=[0 0 1] vec2=[0 -1 0] target=[1] prec=[1000]")));

    // Position task
    mlr::String str;
    ors::Vector target = ors::Vector(0, 0, 0.25);
    str << "map=pos ref1=endeffL ref2=alvar_10 vec2=[" << target.x << ", " << target.y << ", " << target.z << "] PD=[1., 1, 1., 1.]";
...
//  }
  ...
  mlr::Array<uint> joints_to_test = lockbox.locked_joints;
