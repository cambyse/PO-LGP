...
bool TreeControllerClass::init(pr2_mechanism_model::RobotState *robot, ros::NodeHandle &n)
{
  ...
  q_ref = arr(controlIdx.d0);
  qdot_ref = arr(controlIdx.d0);   qdot_ref.setZero();
  ...
  jointState_publisher = n.advertise<marc_controller_pkg::JointState>("jointState", 1);
  jointReference_subscriber = n.subscribe("jointReference", 1, &TreeControllerClass::jointReferenceSubscriber, this);
...
void TreeControllerClass::update()
{
  ...
  joint_pub_state.q = VECTOR(q);
  joint_pub_state.qd = VECTOR(qd);
  jointState_publisher.publish(joint_pub_state);
...
//  q_ref = q + tau_control*qdot_ref;
//  qdot_ref = qd + tau_control*qdd;

  // PD on q_ref!
  p_effort = (Kp % (q_ref - q));
  d_effort = (Kd % (qdot_ref - qd));
  u = p_effort + d_effort;
  ...
  if (LOGGING && (index < StoreLen)) {
    q_bk[index] = q;
    qd_bk[index] = qd;
    des_q_bk[index] = q_ref;
    des_qd_bk[index] = qdot_ref;
    u_bk[index] = u;
...
bool TreeControllerClass::setPosTarget(marc_controller_pkg::SetPosTarget::Request &req, marc_controller_pkg::SetPosTarget::Response &resp){
...
bool TreeControllerClass::getPosTarget(marc_controller_pkg::GetPosTarget::Request &req, marc_controller_pkg::GetPosTarget::Response &resp){
...
bool TreeControllerClass::setVecTarget(marc_controller_pkg::SetVecTarget::Request &req, marc_controller_pkg::SetVecTarget::Response &resp){
  for(uint i =0;i<taskPos->y_ref.d0;i++) {
...
bool TreeControllerClass::getVecTarget(marc_controller_pkg::GetVecTarget::Request &req, marc_controller_pkg::GetVecTarget::Response &resp){
...
bool TreeControllerClass::getTaskState(marc_controller_pkg::GetTaskState::Request &req, marc_controller_pkg::GetTaskState::Response &resp){
  //  world->kinematicsPos(state,NoArr,world->getBodyByName("endeffR")->index);
  //  world->kinematicsVec(stateVec,NoArr,world->getBodyByName("endeffR")->index);
  state = taskPos->y;
  stateVec = taskVec->y;
...
bool TreeControllerClass::setJointGains(marc_controller_pkg::SetJointGains::Request &req, marc_controller_pkg::SetJointGains::Response &resp){
...
bool TreeControllerClass::getJointGains(marc_controller_pkg::GetJointGains::Request &req, marc_controller_pkg::GetJointGains::Response &resp){
...
bool TreeControllerClass::getJointState(marc_controller_pkg::GetJointState::Request &req, marc_controller_pkg::GetJointState::Response &resp){
  resp.q.resize(q.d0);
  resp.qd.resize(q.d0);
...
bool TreeControllerClass::getFilterGains(marc_controller_pkg::GetFilterGains::Request &req, marc_controller_pkg::GetFilterGains::Response &resp){
  resp.q_filt = q_filt;
  resp.qd_filt = qd_filt;
  return true;
}
bool TreeControllerClass::setFilterGains(marc_controller_pkg::SetFilterGains::Request &req, marc_controller_pkg::SetFilterGains::Response &resp){
  q_filt = req.q_filt;
  qd_filt = req.qd_filt;
  return true;
}
bool TreeControllerClass::getTaskGains(marc_controller_pkg::GetTaskGains::Request &req, marc_controller_pkg::GetTaskGains::Response &resp){
...
bool TreeControllerClass::setTaskGains(marc_controller_pkg::SetTaskGains::Request &req, marc_controller_pkg::SetTaskGains::Response &resp){
...
bool TreeControllerClass::startLogging(marc_controller_pkg::StartLogging::Request &req, marc_controller_pkg::StartLogging::Response &resp){
  storage_index_ = 0;
  LOGGING=true;
  return true;
}
bool TreeControllerClass::stopLogging(marc_controller_pkg::StopLogging::Request &req, marc_controller_pkg::StopLogging::Response &resp){
  LOGGING = false;
