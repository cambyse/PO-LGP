    ...
    def __init__(self):
        ...
        with self.sm:
            smach.StateMachine.add(
                ...
                transitions={'succeeded': 'PICK_OOI', 'aborted': 'INIT'}
            )
            smach.StateMachine.add(
                'PICK_OOI',
                smach_ros.SimpleActionState('pick_ooi', msgs.PickOOIAction),
                transitions={'succeeded': 'GOTO_OOI', 'aborted': 'INIT'}
            )
            smach.StateMachine.add(
                'GOTO_OOI',
                smach_ros.SimpleActionState('goto_ooi', msgs.GotoOOIAction),
                # transitions={'succeeded':'GOTO_OOI', #'aborted':'GOTO_OOI'}
                transitions={
                    'succeeded': 'ARTICULATE_OOI', 'aborted': 'PICK_OOI'}
            )
            smach.StateMachine.add(
                'ARTICULATE_OOI',
                ...
                transitions={
                    'succeeded': 'OBSERVE_OOI_TRAJ', 'aborted': 'PICK_OOI'}
            )
            smach.StateMachine.add(
                'OBSERVE_OOI_TRAJ',
                smach_ros.SimpleActionState(
            ...
            smach.StateMachine.add(
                ...
                transitions={'succeeded': 'PICK_OOI', 'aborted': 'PICK_OOI'}
            )

        self.sis = smach_ros.IntrospectionServer(
            'tcr_sis_server', self.sm, '/SM_ROOT')
        self.sis.start()
