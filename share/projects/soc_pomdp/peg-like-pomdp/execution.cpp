...
void getTrajectory(arr& x, arr& y, arr& dual, mlr::KinematicWorld& world, arr x0, const arr& model, bool stickyness, uint horizon){
  ...
  Task *pos = P.addTask("position", new TaskMap_Default(posTMT, world, "endeff", NoVector));
  pos->setCostSpecs(P.T, P.T,conv_vec2arr(target->X.pos), 2e5);

  Task *vel = P.addTask("position_vel", new TaskMap_Default(posTMT, world, "endeff", NoVector));
  vel->map.order=1;
  ...
  Task *cons = P.addTask("planeConstraint", new PlaneConstraint(world, "endeff", ARR(0,0,-1, world.getBodyByName("table")->X.pos.z + 0.02)));//0.2 is table width  //0.05 above table surface to avoid slippery
...
void POMDPExecution(FSC fsc, mlr::KinematicWorld& world, int num, double est){
    ...
    CtrlTask *pd_y =  MC.addPDTask("position", .1, .8, new TaskMap_Default(posTMT, world, "endeff", NoVector));
    pd_y->prec = 10.;
