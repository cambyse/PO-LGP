...
void getTrajectory(arr& x, arr& y, arr& dual, ors::KinematicWorld& world, arr x0, const arr& model, bool stickyness, uint horizon){
  ...
  //SET INITIAL STATE: Model parameter (height, position, size,...), and initial joint's position
    ...
    world.getBodyByName("table")->X.pos.y   = model(1);
    world.getBodyByName("target")->X.pos.y  = model(1);
  ...
  TaskCost *pos = P.addTask("position", new DefaultTaskMap(posTMT, world, "endeffR", NoVector));
  P.setInterpolatingCosts(pos, MotionProblem::finalOnly,ARRAY(target->X.pos.x,target->X.pos.y,target->X.pos.z), 2e5);

  TaskCost *vel = P.addTask("position_vel", new DefaultTaskMap(posTMT, world, "endeffR", NoVector));
  vel->map.order=1;
  ...
  TaskCost *cons = P.addTask("planeConstraint", new PlaneConstraint(world, "endeffR", ARR(0,0,-1, world.getBodyByName("table")->X.pos.z + 0.02)));//0.2 is table width  //0.05 above table surface to avoid slippery
...
void POMDPExecution(FSC fsc, ors::KinematicWorld& world, int num, double est){
    ...
    ors::Shape *endeffR = world.getShapeByName("endeffR");
    ...
    for(uint t=0;t<x.d0 + 100;t++){
      ...
      //scan all next node
       if(t<y.d0-2){
          ...
          if(endeffR->X.pos.x < table->X.pos.x - 0.5){
              observation_x = endeffR->X.pos.x;

          }
          /////////////////////////////////////////////////////////


          double current_pos_eff_z = endeffR->X.pos.z;
          double current_pos_eff_x = endeffR->X.pos.x;
      ...
      if(pd_c->desiredApproach.y.N){
        ...
        if(pd_c->desiredApproach.y_ref(0)==0. && d<1e-2){
         ...
         //   if(change_force){
        //        observation_x = endeffR->X.pos.x; //found the  change of edge
