...
void TEST(Outlier){
  ...
  double p_prior=.5;
  arr sigma(2);
  sigma(0) = mlr::getParameter<double>("sigma",1.);
  sigma(1) = mlr::getParameter<double>("outlierSigma",10.);
  arr q(n),w(n),e(n);
  q=mlr::getParameter<double>("outlierRate",.1); w=1.;
  for(uint r=0;r<10;r++){
    ridgeRegression(beta, Phi, y, mlr::getParameter<double>("ridge",1e-10), &w);
