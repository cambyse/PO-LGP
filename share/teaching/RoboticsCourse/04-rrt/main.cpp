...
void optim(){
  ...
  for(uint k=0;k<20;k++){
    optGaussNewton(x, Convert(P), OPT(stopIters=1, verbose=2, useAdaptiveDamping=.0, maxStep=.1, stopTolerance=1e-2));
    plotEffTraj(S, x);
    S.watch();
  }
#endif

  optGaussNewton(x, Convert(P), OPT(stopIters=1000, verbose=2, useAdaptiveDamping=.0, maxStep=.1, stopTolerance=1e-2));
  MT::save(x,"q.optim");
