...
int get_list_index(FactorGraph& fg, Factor* f);
...
MessagePair* getMessagePair(const Factor* f1, const Factor* f2){
  uint i;
  FOR1D(f1->messages, i){
...
Factor* getMessage(const Factor* f_from, const Factor* f_to){
  MessagePair* msg_pair = getMessagePair(f_from, f_to);
...
void writeMessage(const Factor* f_from, const Factor* f_to, ostream& os){
  os <<*getMessage(f_from, f_to);
}


int get_list_index__orig(FactorGraph& fg, Factor* f_orig){
  return fg.F.findValue(f_orig);
}

uint get_list_index_unsigned__orig(FactorGraph& fg, Factor* f_orig){
  uint i=0;
  while(i < fg.F.N){
    if(fg.F(i) == f_orig)
      break;
  ...
  CHECK(i<fg.F.N, "Factor over " <<f_orig->varIds <<" not found in FactorGraph!" <<endl);
  return i;
}
...
Variable::Variable(uint _dim,const char *_name){
  id=VarCount++;
  dim=_dim;
  name=_name;
}

Variable::Variable(uint _dim,const char *_name,uint _id){
  id=_id;
  dim=_dim;
  name=_name;
}
...
void disconnectFactor(Factor &f){
  uint i;
  Variable *v;
...
/*Factor::Factor(const uintA& vars){
  messages.memMove=true;
  initFactor(*this,vars);
  setOne();
}

Factor::Factor(const uintA& vars,const char* text){
  messages.memMove=true;
  initFactor(*this,vars);
  setText(text);
}
...
void Factor::operator=(const Factor& q){
  initFactor(*this,q.variables);
  P=q.P; logP=q.logP;
}

void Factor::setP(const arr& p){
  P = p;
...
void Factor::setText(const char* str){
  P.setText(str);
  CHECK(P.N==product(dim),"Factor set with ill-dimensioned array");
  P.reshape(dim);
  logP = 0.;
...
void Factor::setEvidence(uint e){
  P=0.;
  P(e)=1.;
  logP=0.;
}

bool Factor::operator==(Factor& q){
  //uint i=P.maxIndex();
...
void Factor::getP(arr& p) const{
  p = ::exp(logP)*P;
}

void Factor::write(std::ostream& os,bool brief) const{
  os <<"factor " <<name <<" (";
...
void Factor::writeNice(std::ostream& os) const{
  uint i;
  os <<"[ ";
  FOR1D(variables, i)
    os<<variables(i)->name<<"  ";
...
void Factor::writeExtremelyNice(std::ostream& os) const {
...
void Factor::checkCondNormalization(uint left,double tol){
  tensorCheckCondNormalization_with_logP(P,left, logP, tol);
}

uint Factor::numberNonZeroEntries() {
  uint num = 0;
  uint i;
  FOR_ALL(P, i) {
...
MessagePair::MessagePair(Factor *_f1,Factor *_f2){
  init(_f1,_f2);
}

MessagePair::MessagePair(Variable *_v1,Variable *_v2,Factor *_v_to_v_fac){
  init(_v1,_v2,_v_to_v_fac);
}

MessagePair::MessagePair(Factor *_f1,Variable *_v2){
  init(_f1,_v2);
}
...
void MessagePair::init(Factor *_f1,Factor *_f2){
  f1 = _f1;
  f2 = _f2;
  v1 = v2 = NULL;
  v_to_v_fac = NULL;
...
void MessagePair::init(Variable *_v1,Variable *_v2,Factor *_v_to_v_fac){
  f1 = f2 = NULL;
  v1 = _v1;
  v2 = _v2;
...
void MessagePair::init(Factor *_f1,Variable *_v2){
  f1 = _f1;
  f2 = NULL;
  v1 = NULL;
  v2 = _v2;
...
void MessagePair::write(ostream& os) const{
  os <<"MessagePair ";
...
void MessagePair::writeIds(ostream& os) const{
  NIY;
  //MT::IOraw=true;
...
void checkMessagePairConsistency(MT::Array<MessagePair*> messages){
  uint i, k;
  int idx;
  Factor* f;
  FOR1D(messages, i){
...
void FactorGraph::write(std::ostream& os, bool writeBeliefs, bool writeMessages) const{
  uint i;
  os <<"<FactorGraph>"<<endl;
...
void FactorGraph::writeNice(std::ostream& os) const {
  uint i, j;
...
void FactorGraph::writeMessagePairs(std::ostream& os) const {
  uint s;
...
void FactorGraph::writeVariableBeliefs(std::ostream& os) const{
  uint i;
...
void FactorGraph::checkCondNormalization_B_c(double tol){
  uint i;
  FOR1D(B_c, i){
...
void FactorGraph::resetMessages(){
  uint i;
  FOR1D(messages, i){
...
void FactorGraph::resetVariableFactors(){
  uint i;
  FOR1D(F_v, i){
    F_v(i)->setOne();
  }
...
void FactorGraph::resetVariableBeliefs(){
  uint i;
  FOR1D(B_v, i){
    B_v(i)->setOne();
  }
...
double FactorGraph::computeBeliefs(){
...
Factor* FactorGraph::getBelief(Factor* f_orig){
  int idx;
...
void FactorGraph::addF2Bmap(Factor* f, Factor* b){
  F2B[f] = b;
}

Factor* FactorGraph::getBelief_fast(Factor* f_orig){
  return F2B[f_orig];
}

void FactorGraph::setV2Fv(){
...
void FactorGraph::addV2Fmap(Factor* f){
  V2F[f->varIds(0)].append(f);
}


// void FactorGraph::checkFaithfulness(){
//   uint DEBUG=0;
...
void get_vars(FactorGraph& fg, uintA& varIds){
  varIds.clear();
  uint i;
...
void LoopyBP::constructBipartiteFactorGraph(FactorGraph& fg, const FactorList& factors){
  ...
  FOR1D(fg.F, i){
    ...
    FOR1D(f->varIds, j){
      v = getvar(f->varIds(j));
      s = new MessagePair(f, v->factors.last());
    ...
    FOR1D(fg.F, k){
      if(fg.F(k)->variables.findValue(fg.V(i)) >= 0){
        ...
        if(fg.F(k)->variables(0) == fg.V(i))
          s = new MessagePair(fg.F(k), f);
        else
          s = new MessagePair(f, fg.F(k));
        fg.messages.append(s);
      }
    }
    // beliefs
...
// Marc's eq (5) and (7)
void computeMessage_noDiv(Factor& f_from, Factor& f_to){
  uint DEBUG = 0;
...
void computeMessage_withDiv(Factor& f_from, Factor& f_to){
  uint DEBUG = 0;
  if(DEBUG>0){cout<<"calcMessage [START]"<<endl;}
  // find MessagePair
  MessagePair* s = getMessagePair(&f_from, &f_to);
  if(&f_from == s->f2){
...
// double distributeMessages(FactorGraph& fg, Factor& f, MsgCalc calcMsgType){
//   uint DEBUG = 0;
//   uint i, index;
...
void checkJunctionTreeProperty_dfs(Factor* node, Factor* parent, FactorGraph& junctionTree, boolA& containsId, Variable *id){
  int DEBUG = 0;
  uint i;
...
#ifndef MT_NOCHECK
      MessagePair* s = getMessagePair(node, neighbors(i));
#endif
...
void JunctionTree::checkJunctionTreeProperty(FactorGraph& junctionTree){
  int DEBUG = 0;
  if(DEBUG > 0)
...
void JunctionTree::addEvidence(FactorGraph& junctionTree, Factor& evid){
  ...
  FOR1D(junctionTree.F, f){
    ...
    if(varSection.N > 0){
      MessagePair* s = new MessagePair(junctionTree.F(f), &evid);
...
void recursiveCollectEvidence(Factor* node, Factor* parent, FactorGraph& junctionTree){
  uint DEBUG = 0;
  if(DEBUG > 0)
  ...
  FOR1D(neighbors, i){
    ...
    if(node == s->f1){
      ...
      Factor* b_to = junctionTree.B_c(get_list_index_unsigned__orig(junctionTree, s->f1));
      passMessage(*(s->f2), *(s->f1), *b_to, NO_DIV);
    }
    // message 12
    else {
      if(DEBUG > 0) cout <<"Pass message " <<s->f1->varIds <<" --> " <<node->varIds <<endl;
      Factor* b_to = junctionTree.B_c(get_list_index_unsigned__orig(junctionTree, s->f2));
...
void recursiveDistributeEvidence(Factor* node, Factor* parent, FactorGraph& junctionTree){
  uint DEBUG = 0;
  if(DEBUG > 0)
...
void JunctionTree::collectAndDistributeInference(FactorGraph& junctionTree){
...
double LoopyBP_obsolete::passAllEdges(FactorGraph& fg, PassType type){
  if(type == PARALLEL){
...
double LoopyBP_obsolete::passAllEdges_parallel(FactorGraph& fg){
  uint DEBUG =0;
  // (1) cliques --> vars
  uint i;
...
void LoopyBP_obsolete::shoutMessages(Factor& f, MsgCalc calcMsgType){
  uint i;
  FOR1D(f.messages, i){
    Factor* f_to;
    if(f.messages(i)->f1 == &f)
...
void LoopyBP::getVarBeliefs(MT::Array<Factor>& beliefs){
  uint i;
  beliefs.resize(vars.N);
  for(i=0;i<vars.N;i++) collectBelief(beliefs(i),vars(i),NULL);
}

void LoopyBP::getVarBelief(Factor& belief,Variable *v){
...
void loopyBP_bipartite(const VariableList& vars,const FactorList& facs,uint T){
  ...
  MT::Array<Factor> beliefs(vars.N);
  uint t;
  for(t=0;t<T;t++){
...
void tree2FactorGraph(FactorGraph& fg,const MT::Array<TreeNode>& tree){
  uint i,N=tree.N;
  fg.V.resize(N);
  fg.F.resize(N);
  ...
  for(i=0;i<N;i++){    //messages
    if(tree(i).parent>=0) fg.messages.append(new MessagePair(fg.F(i),fg.F(tree(i).parent)));
  }
  fg.resetMessages();
...
void treeInference(MT::Array<arr>& posteriors, const Tree& tree){
  ...
  getVariableBeliefs(posteriors, model.V);
}
...
void recomputeBatchOfMessages(MessagePairList &msgs,bool invert_order){
  uint i,N=msgs.N;
...
void treeInference(const Factor *root,bool check_consitency){
