...
double p_to_ratio(double p){
  if(p<.5){ if(p<1e-10) p=1e-10; return .5*log(p/(1.-p)); }
  else    { p=1.-p; if(p<1e-10) p=1e-10; return -.5*log(p/(1.-p)); }
}
...
void BinaryBPNet::getFG(BinaryPairFG &FG,bool betheForm){
  ...
  if(betheForm)  for_list(i,e,edges){  pairExp_to_Table(FG.f_ij[i](), e->J, e->from->theta, e->to->theta);  }
  else           for_list(i,e,edges){  pairExp_to_Table(FG.f_ij[i](), e->J, 0., 0.);  }
