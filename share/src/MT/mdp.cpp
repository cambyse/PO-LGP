...
void mdp::readMDP(MDP_structured& mdp, const char *filename){
  ...
  if(!strcmp(ext,"dp_arr")){ MDP mdp_flat; readMDP_arr(mdp_flat, filename); convert(mdp,mdp_flat); }
  else if(!strcmp(ext,".POMDP")){ MDP mdp_flat; readMDP_POMDP(mdp_flat, filename); convert(mdp,mdp_flat); }
  else if(!strcmp(ext,"mdp_fg")) readMDP_fg(mdp, filename);
...
void mdp::readMDP_arr(MDP& mdp, const char *filename, bool binary){
  ...
  arr(&mdp.gamma,1)().readTagged(is,"gamma");
}
...
void mdp::readMDP_ddgm_tabular(MDP_structured& mdp, const char *filename){
  ...
  for(;;){
    ...
    else if(mdp.ctrlVars.findValue(v)!=-1){ HALT("there should be no CPT for controlled vars!"); }
    else mdp.transFacs.append(f);
  }
...
void mdp::initHierarchical(uint exits,FSC_lev2& fsc){
  ...
  for(i=0;i<d0;i++){
    if(i>=d0-exits){ fsc.Pe0(1,i)=1.; fsc.Pe0(0,i)=0.; }
    else           { fsc.Pe0(1,i)=0.; fsc.Pe0(0,i)=1.; }
  }
