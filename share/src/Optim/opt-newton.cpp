...
OptNewton::OptNewton(arr& _x, ScalarFunction& _f,  OptOptions _opt):
  x(_x), f(_f), opt(_opt){
  alpha = 1.;
  lambda = opt.damping;
  evals=0;
...
//        alpha = pow(alpha, 0.5);
    ...
    //stopping criteria
#define STOPIF(expr) if(expr){ if(o.verbose>1) cout <<"\t\t\t\t\t\t--- stopping criterion='" <<#expr <<"'" <<endl; stoppingCriteron=true; }
    STOPIF(lambda<2. && absMax(Delta)<o.stopTolerance);
    STOPIF(lambda<2. && alpha*absMax(Delta)<1e-3*o.stopTolerance);
    STOPIF(evals>=o.stopEvals);
    STOPIF(it>=o.stopIters);
