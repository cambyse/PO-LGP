...
/* this function writes the data in a format to be read by AMPL
 * the created file might still need minor tweaks */
int writeAMPL( char *filename ){
  FILE *file;
  int a, i, j, obs, k;
...
/* This code also simulates the addition of an absorbing state */

/* states and next states */
  fprintf( file, "set States:= ");
  for( i = 0; i < gNumStates; i++)
	  fprintf(file, "%d ", i);
/* fprintf(file, "%d ", i); */ /* absorbing state */
  fprintf( file, ";\n");
  
/*  fprintf( file, "set NextStates:= ");
  for( i = 0; i < gNumStates; i++)
	  fprintf(file, "%d ", i);
  fprintf( file, ";\n");
  */
...
/* observations */
fprintf( file, "set Observations:= ");
  for( i = 0; i < gNumObservations; i++)
	  fprintf(file, "%d ", i);
  /* fprintf(file, "%d ", i); */ /* observation corresponding to the absorbing state */ 	  
  fprintf( file, ";\n");


/* actions */
fprintf( file, "set Actions:= ");
  for( i = 0; i < gNumActions; i++)
	  fprintf(file, "%d ", i);
  fprintf( file, ";\n");

/* gamma */
  fprintf( file, "param gamma := %.6lf;\n", gDiscount);  /* absorbing states */

/* initial belief state, b_0 */  
  fprintf(file, "param b_0 :=");
  for( i=0; i < gNumStates; i++)
	  fprintf(file, " %d %.6lf", i, gInitialBelief[i]);
/*  fprintf(file, " %d 0.0", i); */  /* absorbing state */ 
  fprintf(file, ";\n");

/* reward */  
  fprintf( file, "\n\n# PARAM REWARD\n");
  fprintf( file, "param R : ");
  for( a = 0; a < gNumActions; a++)
	  fprintf(file, "%d ", a);
  fprintf( file, ":=\n");
  for( i = 0; i < gNumStates; i++)
  {
	  ...
	  for( a=0; a < gNumActions; a++)
	  {
		  /* check if this reward exists if not put 0*/
		  short found = 0;
		  for(j=Q->row_start[a]; j < Q->row_start[a] + Q->row_length[a]; j++)
		  {
			  if(Q->col[j] == i) /* if actions and state match */
			  {
				found = 1;
	  			fprintf(file, "%.6lf ", Q->mat_val[j]);
			  }
		  }
		  if(!found)
	  		fprintf(file, "0 ");
  ...
  /* add 0 reward for this new state */ 
/*   fprintf(file, "%d ", i); 
  for(a=0;a<gNumActions;a++)
  	fprintf(file, "0.0 ");*/ 
  fprintf(file, ";\n");
...
/* Ps */
	fprintf(file, "\n\n# PARAM Ps{s_ in NextStates, s in States, a in Actions};\n");
	fprintf(file, "param Ps :=\n");
	for(a=0;a<gNumActions;a++)
	{
		fprintf(file, "\n[*,*,%d]:\n", a);
		for(i=0;i<gNumStates;i++)
			fprintf(file, "%d ", i);
		/* fprintf(file, "%d ", i); */ /* absorbing states */
		fprintf(file, ":=\n");
		for(k=0;k<gNumStates;k++) /* end state */
		{
			/* this is wasteful ... */
			fprintf(file, "%d ", k);
			for(i=0;i<gNumStates;i++)  /* start state */
			{
				short found = 0;
				for(j=P[a]->row_start[i]; j < P[a]->row_start[i] + P[a]->row_length[i];j++)
				{
					if(P[a]->col[j] == k && !found)
					{
						fprintf(file, "%.6lf ", P[a]->mat_val[j]*gDiscount); /* absorbing state */
						found =1;
					}
				}
				if(!found)
			...
			/* fprintf(file, "0 ", k); */ /* absorbing state */
			fprintf(file, "\n");
		}
#if 0		
		fprintf(file, "%d ", k); /* absorbing state */
		for(k=0;k<gNumStates;k++)
			fprintf(file, "%.6lf ", 1.0-gDiscount); /* absorbing states */
		fprintf(file, "1.0 ");	/* absorbing states */
#endif 
...
/* Po */
	/* param Po{o in Observations, s_ in NextStates, a in Actions}; */
	fprintf(file, "\n\n# PARAM Po{o in Observations, s_ in NextStates, a in Actions};\n");
	fprintf(file, "param Po :=\n");
	for(a=0;a<gNumActions;a++)
	{
		fprintf(file, "\n[*,*,%d]:\n", a);
		for(i=0;i<gNumStates;i++)
...
/*		fprintf(file, "%d ", i); */ /* absorbing state */
		fprintf(file, ":=\n");
		for(obs=0;obs<gNumObservations;obs++) /* observations state */
		{
			/* this is wasteful ... */
			fprintf(file, "%d ", obs);
			for(i=0;i<gNumStates;i++)  /* next state */
			{
				short found = 0;
				for(j=R[a]->row_start[i]; j < R[a]->row_start[i] + R[a]->row_length[i];j++)
				{
					if(R[a]->col[j] == obs && !found)
					{
						fprintf(file, "%.6lf ", R[a]->mat_val[j]);
						found =1;
					}
				}
				if(!found)
			...
			/* fprintf(file, "0 "); */ /* absorbing state */
			fprintf(file, "\n");
		}
#if 0		
		fprintf(file, "%d ", obs); /* absorbing state */
		for(k=0;k<gNumStates;k++)
			fprintf(file, "0 "); /* absorbing states */
		fprintf(file, "1.0 ");	/* absorbing states */
#endif		
