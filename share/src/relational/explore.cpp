...
double RuleExplorer::calcRuleConfidence(uint rule_id) {
  if (fixedActions.findValue(getRules().elem(rule_id)->action) >= 0)  // fixed actions
    return 99.;
  if (ruleReasoning::isDefaultRule(getRules().elem(rule_id))) // default rule:  never confident
    return 0.;
  return getNumberOfCoveredExperiences(rule_id);
}


void RuleExplorer::calcExploreWeights(arr& explore_weights, Atom* taboo_action) {
  uint DEBUG = 2;
  ...
  logicReasoning::sort(tabooed__direct_explore__actions);
  logicReasoning::sort(strongly_tabooed__direct_explore__actions);
...
void RuleExplorer::updateActionAndStateMeasures() {
  uint DEBUG = 2;
  #ifdef ENFORCED_SILENCE
  DEBUG = 0;
  #endif
  ...
  const RuleSet& rules = getRules();
  if (representation == relational)
    ruleReasoning::coveringRules(actions__covering_rules, rules, possibleGroundActions, current_state);
  else
    ruleReasoning::coveringGroundedRules(actions__covering_rules, rules, possibleGroundActions, current_state);
...
//       FlatExplorer* flat = (FlatExplorer*) this;
...
bool RuleExplorer::stateIsKnown(const SymbolicState& state, const AtomL& ground_actions) {
  uint i;
...
bool RuleExplorer::stateIsKnown_partially(const SymbolicState& state, const AtomL& ground_actions) {
  uint i;
  FOR1D(ground_actions, i) {
...
bool RuleExplorer::actionIsKnown(const SymbolicState& state, Atom* action) {
...
Atom* RuleExplorer::getExploitAction(AtomL& plan, PRADA_Planner* prada, const SymbolicState& state) {
  uint DEBUG = 0;
  #ifdef ENFORCED_SILENCE
  DEBUG = 0;
  #endif
...
//   cout<<"PRADA's used actions:   ";  writeNice(prada->ground_actions);  cout<<endl;
  ...
  if (plan.N>0  &&  plan(0) != logicObjectManager::getAtom_doNothing())  // TODO, hmm, maybe doNothing-action should lead to a HALT(.)?
    return plan(0);
  else
    return NULL;
}
...
class PRADA_Reward__Rmax : public PRADA_Reward__Unknown_Context {
  ...
  public :
    ...
    PRADA_Reward__Rmax(Reward* exploit_reward, AtomL& _ground_actions, uint _depth)
...
Atom* RuleExplorer::getExploreAction_planned__contexts(AtomL& explore_plan, PRADA_Planner* prada, const SymbolicState& state) {
  uint DEBUG = 1;
  #ifdef ENFORCED_SILENCE
  DEBUG = 0;
  #endif
...
Atom* RuleExplorer::getExploreAction_planned(AtomL& explore_plan, PRADA_Planner* prada, const SymbolicState& state, uint type) {
  if (type == 1) {
...
Atom* RuleExplorer::getExploreAction_direct(PRADA_Planner* prada, const SymbolicState& state, Atom* taboo_action) {
  uint DEBUG = 1;
  #ifdef ENFORCED_SILENCE
  DEBUG = 0;
  #endif
  ...
  Atom* action = possibleGroundActions(TL::basic_sample(explore_weights));
...
Atom* RuleExplorer::decideAction(const SymbolicState& state, NID_Planner* planner, uint behavior_type, bool use_known_state_partial) {
  uint DEBUG = 2;
  #ifdef ENFORCED_SILENCE
  DEBUG = 0;
  #endif
...
//   }
  ...
  // ----------------------------
    ...
    if ((use_known_state_partial && current_state_is_known_partially)
      ...
      // No good plan has been found: plan in modified DBN.
        ...
        if (DEBUG>0) {cout<<"*** PLANNED EXPLORE TRY. ***"<<endl;   cerr<<"*** PLANNED EXPLORE TRY. ***"<<endl;}
          AtomL explore_plan;
          action = getExploreAction_planned(explore_plan, (PRADA_Planner*) planner, state);
        if (action != NULL) {
    ...
    if (action == NULL) {
      ...
      action = getExploreAction_direct((PRADA_Planner*) planner, state, bad_action);
      moves_explore_direct.append(visited_pre_states.N);
      moves.append(MOVE_TYPE__EXPLORE_DIRECT);
    }
  }
  ...
  // ----------------------------
    ...
    // build r-max reward
    PRADA_Reward__Rmax* rmax_reward = new PRADA_Reward__Rmax(exploit_reward, possibleGroundActions, PLANNED_EXPLORE__HORIZON);
    
    MaximizeFunctionReward fake_reward;
    ((PRADA_Planner*) planner)->setReward(exploit_reward, rmax_reward);
    ...
    if (action != NULL) {
      ...
      action = getExploreAction_direct((PRADA_Planner*) planner, state, NULL);
      moves.append(MOVE_TYPE__EXPLORE_DIRECT);
    }
...
void RuleExplorer::addObservation__helper(SymbolicState* state_pre, Atom* action, SymbolicState* state_post) {
...
void RuleExplorer::addObservations(const Trial& trial) {
  uint i;
...
void RuleExplorer::updateLogicEngineConstants() {
  ...
  FOR1D(potential_ground_actions, i) {
    if (potential_ground_actions(i)->args.N == 2  &&  potential_ground_actions(i)->args(0) == potential_ground_actions(i)->args(1))
      continue;
    if (potential_ground_actions(i)->pred->id != DEFAULT_ACTION_PRED__ID) {
...
AbstractRuleExplorer::AbstractRuleExplorer(double complexity_penalty_coeff,
  ...
  FOR1D(logicObjectManager::p_actions, i) {
    if (logicObjectManager::p_actions(i)->id == DEFAULT_ACTION_PRED__ID)
      continue;
...
uint AbstractRuleExplorer::action_to_learner_id(Atom* action) {
  uint i;
  FOR1D(modeledActions, i) {
    Substitution sub;
    ...
    if (learners_uptodate(i)) {
      ...
      if (!always_re_learning) {
        ...
        FOR1D(experiences_per_modeledAction(i), k) {
          ...
          SymbolicState& s_pre = all_experiences(experience_id)->pre;
          Atom* action = all_experiences(experience_id)->action;
          uint rule_idx = ruleReasoning::uniqueAbstractCoveringRule_groundedAction(rulesC.rules, s_pre, action);
          ...
          ruleReasoning::coveringOutcomes(rulesC.rules.elem(rule_idx), s_pre, action, all_experiences(experience_id)->post, covering_outcomes);
          uint o;
      ...
      if (DEBUG>1) {
        ...
        FOR1D(old_experience_partitions, k) {
          ...
          if (rules_ids.N == 1) {
            write(rulesC.rules.elem(rules_ids(0))->context);
          }
          else
            cout<<"Default rule";
      ...
      Rule* old_covering_rule_for_latest_example = NULL;
      uint old_covering_rule_for_latest_example__id = ruleReasoning::uniqueAbstractCoveringRule_groundedAction(rulesC.rules, *visited_pre_states.last(), visited_actions.last());
      if (old_covering_rule_for_latest_example__id != 0) {
        old_covering_rule_for_latest_example = new Rule;
      ...
      RuleSetContainer rulesC_for_action;
...
//       if (DEBUG>2) {cout<<"Yes, new rules survived sanity check."<<endl;}
      if (DEBUG>2) {
        ...
        if (DEBUG>3) {write(action_experiences);}
      ...
      FOR1D_(rulesC_for_action.rules, k) {
        if (ruleReasoning::isDefaultRule(rulesC_for_action.rules.elem(k)))
          continue;
...
//       if (DEBUG>2) {cout<<"Yes, AFTER rule-set survived sanity check!"<<endl;}
      ...
      if (!partitions_changed) {
        Rule* new_covering_rule_for_latest_example = NULL;
...
//   cout<<"Survived sort()"<<endl;
  ...
  if (current_state.lits_prim.N > 0) {
    ...
    ruleReasoning::ground_with_filtering(confident_ground_rules, confident_rules, logicObjectManager::constants, current_state);
  }
  else {
    confident_ground_rules.append(rulesC.rules.elem(0));
  }
  confident_ground_rules.sort_using_args();
  ruleReasoning::removeDoubleLiterals(confident_ground_rules);
  ruleReasoning::checkRules(confident_ground_rules);
  if (DEBUG>1) {
  ...
  if (fixedActions.findValue(action) >= 0) {
    ...
    FOR1D(coveringRuleIDs, i) {
      if (coveringRuleIDs(i) != 0) {
        ...
        uintA covering_outcomes;
        ruleReasoning::coveringOutcomes(rulesC.rules.elem(coveringRuleIDs(i)), *state_pre, action, *state_post, covering_outcomes);
...
double AbstractRuleExplorer::calcRuleConfidence(uint rule_id) {
  ...
  rule_experiences_entropies(rule_id) = RelationalStateGraph::entropy(filtered_actions, filtered_graphs);
  
  uintA rule_involved_objects;
  logicReasoning::getConstants(rulesC.rules.elem(rule_id)->context, rule_involved_objects);
...
void AbstractRuleExplorer::updateActionAndStateMeasures() {
  uint DEBUG = 0;
  #ifdef ENFORCED_SILENCE
  DEBUG = 0;
  #endif
  ...
  FOR1D(possibleGroundActions, i) {
    uintA coveringRuleIDs;
    ruleReasoning::coveringRules_groundedAction(rules, current_state, possibleGroundActions(i), coveringRuleIDs);
...
//       cout<<"--> Regel:  "<<endl;  rules.elem(actions__covering_rules(i))->writeNice();
  ...
  if (density_estimation_type == density_entropy) {
    ...
    RelationalStateGraph graph__current_state(current_state);
...
uint FactoredRuleExplorer::action_to_learner_id(Atom* action) {
  int id = possibleGroundActions.findValue(action);
...
//           uintA rules_ids = rulesC.nonDefaultRules_per_experience(old_experience_partitions(k)(0));
//           if (rules_ids.N == 1) {
//             writeNice(rulesC.rules.elem(rules_ids(0))->context);
//           }
//           else
//             cout<<"Default rule";
//           cout<<endl<<"   "<<old_experience_partitions(k)<<endl;
...
//       
      ...
      // (iii) learn new rules for this action
      RuleSetContainer_ground rulesC_for_action;
      SymbolicExperienceL action_experiences;
      arr action_experience_weights;
      FOR1D(experiences_per_modeledAction(i), k) {
        action_experiences.append(all_experiences(experiences_per_modeledAction(i)(k)));
...
//             if (DEBUG>1) {
//               cout<<"Major experience since context has changed"<<endl;
//               cout<<"old_covering_rule_for_latest_example->context:  ";  writeNice(old_covering_rule_for_latest_example->context);  cout<<endl;
//               cout<<"new_covering_rule_for_latest_example->context:  ";  writeNice(new_covering_rule_for_latest_example->context);  cout<<endl;
//             }
//           }
//         }
//         else if (new_covering_rule_for_latest_example != NULL  &&  old_covering_rule_for_latest_example == NULL) {
  ...
  if (fixedActions.findValue(action) >= 0) {
    ...
    FOR1D(coveringRuleIDs, i) {
      if (coveringRuleIDs(i) != 0) {
        rulesC.experiences_per_rule(coveringRuleIDs(i)).append(all_experiences.N-1);
        uintA covering_outcomes;
        ruleReasoning::coveringOutcomes(rulesC.rules.elem(coveringRuleIDs(i)), *state_pre, *state_post, covering_outcomes);
...
FlatExplorer::FlatExplorer(RuleSet& fixed_rules_for_fixed_actions) 
...
//     rules_hierarchy(rules_hierarchy.N-2).append(ruleReasoning::getDoNothingRule());
//   }
//   else {
    rules_hierarchy.resize(modeledActions.N+1);
//   }
  rules_hierarchy(rules_hierarchy.N-1).append(ruleReasoning::generateDefaultRule());
...
//   CHECK(CALLS__updateLogicEngineConstants_FLAT <= 1, "uiuiui, das muss hier noch angepasst werden, digger; letztlich sind alle learner zurueckzusetzen");
  ...
  // update rule-set
  RuleSet default_rule_wrapper;
  default_rule_wrapper.append(rules_hierarchy.last().elem(0));
//   RuleSet doNothing_rule_wrapper;
...
uint FlatExplorer::action_to_learner_id(Atom* action) {
  NIY;
  return -1;
//   int id = possibleGroundActions.findValue(action);
//   CHECK(id >= 0, "action "<<*action<<" not found");
...
void FlatExplorer::addObservation(SymbolicState* state_pre, Atom* action, SymbolicState* state_post) {
  uint DEBUG = 1;
  #ifdef ENFORCED_SILENCE
  DEBUG = 0;
  #endif
  ...
  FOR1D_(rules_hierarchy(action_id), i) {
    if (ruleReasoning::cover_groundRule_groundedAction(*state_pre, action, rules_hierarchy(action_id).elem(i)))
      break;
  }
  ...
  // (Case 2) Rule exists already
    ...
    Rule* existing_rule = rules_hierarchy(action_id).elem(i);
  ...
  // (Case 3) Build new rule
    ...
    FOR1D(other_lits__positive, i) {
      if (rule->context.N > 18)  // HACK PRADA verarbeitet Kontexte nur bis maximal 18 Konzepten
        break;
      Literal* lit_neg = logicObjectManager::getLiteralNeg(other_lits__positive(i));
    ...
    bool coverage_test =  ruleReasoning::cover_groundRule_groundedAction(*state_pre, action, rules_hierarchy(action_id).elem(rules_hierarchy(action_id).num()-1));
    PRINT(coverage_test);
    CHECK(coverage_test, "");
  }
  if (DEBUG>0) {cout<<"FlatExplorer::addObservation [END]"<<endl;}
...
int FlatExplorer::action_state_to_flat_id(Atom* action, SymbolicState* state) {
  uint i, k;
  ...
  FOR1D(rules_hierarchy, i) {
    if (i < (uint) idx_actions)
      ...
      FOR1D_(rules_hierarchy(i), k) {
        if (ruleReasoning::cover_groundRule_groundedAction(*state, action, rules_hierarchy(i).elem(k)))
          break;
      }
...
void FlatExplorer::get_nonDefaultRules_for_last_experience(uintA& rule_ids) {
...
RelationalStateGraph::RelationalStateGraph(const SymbolicState& _state) : state(_state) {
  // read constants
...
double RelationalStateGraph::distance(const RelationalStateGraph& g1, const Atom& a1,
                         const RelationalStateGraph& g2, const Atom& a2) {
  uint DEBUG = 0;
...
RelationalStateGraph* RelationalStateGraph::createSubgraph(const Atom& action, const RelationalStateGraph& full_graph, const Rule& rule, uint depth) {
  uint DEBUG = 0;
  ...
  uintA drefs;
  ruleReasoning::calcGroundDeicticReferences(drefs, full_graph.state, &action, &rule);
