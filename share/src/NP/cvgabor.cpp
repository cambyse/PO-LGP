...
{
//   // check parameters
   int width = gray_image.d1;
   int height = gray_image.d0;
   int num_responses = num_orientations * num_scales;
   if (num_responses <= 0)
   {
//       ERROR("gabor wavelets cannot be computed, num_responses <= 0");
      return;
   }
   ...
   if (spatial_frequency <= 0)
      spatial_frequency = sqrt(2.0);

  IplImage *ipl_response = cvCreateImage(cvSize(width,height), IPL_DEPTH_8U, 1);
  IplImage *ipl_input = cvCreateImageHeader(cvSize(gray_image.d1, gray_image.d0),IPL_DEPTH_8U,1);
  ipl_input->widthStep = gray_image.d1;
  ipl_input->imageData = (char*)gray_image.p;

   CvGabor *gabor;
   output.resize(num_responses);
   byteA current_output(height, width);
   current_output = 0;
   int counter = 0;
   for (int orientation = 0; orientation < num_orientations; orientation++)
   {
      for (int scale = 0; scale < num_scales; scale++)
      {
        ...
        cur.resize(height, width);
         ...
         gabor = new CvGabor(PI/num_orientations*orientation, scale, sigma, spatial_frequency);
         gabor->conv_img(ipl_input, ipl_response, type);

         np::copy_iplimage2byteA(current_output, *ipl_response);

         for (uint p=0; p<cur.N; p++)
           cur.p[p] = (S) current_output.p[p];
//          for (int y = 0; y < height; y++)
//          {
//              for (int x = 0; x < width; x++)
//                 cur(y,x) = (S)current_output(y,x);
//          }
         counter++;
