...
//  mlr::String& endeffRef = world.getShapeByName(graspRef)->body->inLinks.first()->from->shapes.first()->name;
  ...
  setTask(time-.1, time, new TaskMap_Default(posDiffTMT, world, graspRef, NoVector, object, NoVector), sumOfSqrTT, NoArr, 1e3);
  setTask(time-.1, time, new TaskMap_Default(quatDiffTMT, world, graspRef, NoVector, object, NoVector), sumOfSqrTT, NoArr, 1e3);
...
//#    (MakeJoint rigidZero Hand Obj){ time=1 }
  ...
  if(stepsPerPhase>2){ //otherwise: no velocities
    setTask(time-.2, time-.05, new TaskMap_Default(posTMT, world, object), sumOfSqrTT, {0.,0.,-.1}, 1e1, 1); //move down
...
void KOMO::setPlace(double time, const char* endeffRef, const char* object, const char* placeRef){
  ...
  if(stepsPerPhase>2){ //otherwise: no velocities
    setTask(time-.15, time, new TaskMap_Default(posTMT, world, object), sumOfSqrTT, {0.,0.,-.1}, 1e1, 1);
    setTask(time, time+.15, new TaskMap_Default(posTMT, world, object), sumOfSqrTT, {0.,0.,.1}, 1e1, 1); // move up
  }

  setTask(time-.02, time, new TaskMap_Default(posDiffTMT, world, object, NoVector, placeRef, NoVector), sumOfSqrTT, {0.,0.,.1}, 1e3);
