...
struct Frame{
  FrameConfiguration& F; //every frame is uniquely owned by (listed in) a kinematic configuration
  int ID; //its ID equals its indes in the list
  std::string name;
  Transform pose;

  struct FrameRel  *rel=NULL;
  struct FrameGeom *geom=NULL;
  struct FrameMass *mass=NULL;
};
