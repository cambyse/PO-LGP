...
void Filter::step()
{
  ...
  // Step 1: get subset of objectDatabase that matches the type of the inputs.
  FilterObjects matchedSubset;    // Objects of the same type
  FilterObjects unmatchedSubset;  // Objects of a different type

  for (const FilterObject object : objectDatabase )
  {
    if (object.type == perceptualInputs(0).type )
    {
      matchedSubset.append(object);
    }
    else
    {
      unmatchedSubset.append(object);
    }
  }

  // Create bipartite costs
  costs = createCostMatrix(perceptualInputs, matchedSubset);
  ...
  FilterObjects assignedObjects = assign(perceptualInputs, matchedSubset);


  visualization_msgs::MarkerArray new_markers;
  for (uint i = 0; i < assignedObjects.N; ++i)
  {
    new_markers.markers.push_back(conv_FilterObject2Marker(assignedObjects(i)));
  }
  pub.publish(new_markers);

  // Create the new objectDatabase.
  // Add the FilterObjects of different types back in.
  for (const FilterObject unmatched : unmatchedSubset)
  {
    assignedObjects.append(unmatched);
  }

  // Set the access.
  object_database.set() = assignedObjects;
...
arr Filter::createCostMatrix(const FilterObjects& newObjects, const FilterObjects& oldObjects)
{
  ...
  std::cout << "Dims: " << dims << " Old: " << num_old << " " << num_new << std::endl;
  // Assign costs
