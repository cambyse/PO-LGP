...
double UnconstrainedProblem::fs(arr& g, arr& H, const arr& x){
  arr phi, J;
  double f = P.fs(g, H, x);
  P.fv(phi, (&g?J:NoArr), x);
  CHECK(P.get_m()==phi.N,"this conversion requires phi.N to be m-dimensional");

  //in log barrier case, check feasibility
  if(muLB)     for(uint i=0;i<phi.N;i++) if(phi(i)>0.) return NAN; //CHECK(phi(i)<=0., "log barrier: constraints must be fulfiled!");

  //double f = phi(0); //costs
  if(muLB)     for(uint i=0;i<phi.N;i++) f -= muLB * ::log(-phi(i));  //log barrier
  if(mu)       for(uint i=0;i<phi.N;i++) if(phi(i)>0. || (lambda.N && lambda(i)>0.)) f += mu * MT::sqr(phi(i));  //penalty
  if(lambda.N) for(uint i=0;i<phi.N;i++) if(lambda(i)>0.) f += lambda(i) * phi(i);  //augments

  if(&g){
    //g = J[0]; //costs
    if(muLB)     for(uint i=0;i<phi.N;i++) g -= (muLB/phi(i))*J[i];  //log barrier
    if(mu)       for(uint i=0;i<phi.N;i++) if(phi(i)>0. || (lambda.N && lambda(i)>0.)) g += (mu*2.*phi(i))*J[i];  //penalty
    if(lambda.N) for(uint i=0;i<phi.N;i++) if(lambda(i)>0.) g += lambda(i)*J[i];  //augments
    g.reshape(x.N);
  }

  if(&H){
    ///TODO: Here we assume the hessian of all phi(i) ZERO!!! Only the 2.*J^T J terms are considered (as in Gauss-Newton type)
//    H.resize(x.N,x.N);
//    H.setZero();
    if(muLB)     for(uint i=0;i<phi.N;i++) H += (muLB/MT::sqr(phi(i)))*(J[i]^J[i]);  //log barrier
    if(mu)       for(uint i=0;i<phi.N;i++) if(phi(i)>0. || (lambda.N && lambda(i)>0.)) H += (mu*2.)*(J[i]^J[i]);  //penalty
    if(lambda.N) for(uint i=0;i<phi.N;i++) if(lambda(i)>0.) H += 0.; //augments
    H.reshape(x.N,x.N);
  }
...
void UnconstrainedProblem::augmentedLagrangian_LambdaUpdate(const arr& x){
  ...
  for(uint i=0;i<phi.N;i++) if(phi(i)>0. || lambda(i)>0.) lambda(i) += mu * 2.*phi(i);

  for(uint i=0;i<phi.N;i++) if(lambda(i)<0.) lambda(i)=0.;
...
double PhaseOneProblem::fs(arr& g, arr& H, const arr& x){
  NIY;
}
...
void PhaseOneProblem::fv(arr& metaPhi, arr& metaJ, const arr& x){
  ...
  for(uint i=0;i<phi.N;i++) metaPhi(i) = phi(i)-x.last();    //slack constraints
  metaPhi.last() = -x.last();                                //last constraint

  if(&metaJ){
    metaJ.resize(metaPhi.N, x.N);  metaJ.setZero();
    metaJ(0,x.N-1) = 1.; //cost
    for(uint i=0;i<phi.N;i++) for(uint j=0;j<x.N-1;j++) metaJ(i,j) = J(i,j);
    for(uint i=0;i<phi.N;i++) metaJ(i,x.N-1) = -1.;
    metaJ(phi.N, x.N-1) = -1.;
  }
}
