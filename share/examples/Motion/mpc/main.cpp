...
void plotTraj(arr &x, double dt) {
  ...
  gnuplot("set term wxt 1 title 'position x'");
  ...
  gnuplot("set term wxt 2 title 'velocity xd'");
  ...
  gnuplot("set term wxt 3 title 'acceleration xdd'");
...
void scenario2() {
  ...
  //** Replan from index 250 **//
  uint i = 250;
  arr x0 = xRef[i];
  arr v0 = (xRef[i]-xRef[i-1])/dt;
