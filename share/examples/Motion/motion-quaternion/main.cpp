...
//  G.gl().watch();
  ...
  Task *c;
  c = MP.addTask("transitions", new TransitionTaskMap(G), sumOfSqrTT);
  c->map.order=2; //make this an acceleration task!
  c->setCostSpecs(0, MP.T, {0.}, 1e0);

  c = MP.addTask("position", new DefaultTaskMap(posDiffTMT, G, "endeff", NoVector, "target"), sumOfSqrTT);
  c->setCostSpecs(MP.T, MP.T, {0.}, 1e3);

  c = MP.addTask("quat", new DefaultTaskMap(quatDiffTMT, G, "endeff", NoVector, "target"), sumOfSqrTT);
  c->setCostSpecs(MP.T, MP.T, {0.}, 1e3);
