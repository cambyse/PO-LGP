...
int main(int argn, char **argv) {
  ...
  SystemDescription S;
  for(uint i=0;i<N;i++) S.newVar<int>(STRING("int"<<i));
  for(uint i=1;i<N;i++) S.newModule(STRING("S"<<i-1), "PairSorter", ARRAY(S.variables(i-1), S.variables(i)));
  S.report();

  engine().create(S);

  for(uint i=0;i<S.variables.N;i++) S.getVar<int>(i) = MT::rnd(100);

  for(uint k=0;k<20;k++){
    for(uint i=0;i<S.variables.N;i++) cout <<S.getVar<int>(i) <<' ';  cout <<endl;
    engine().step(S);
    MT::wait(.1);
  }
