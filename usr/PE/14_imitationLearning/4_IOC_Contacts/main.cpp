...
struct IOC:ConstrainedProblem {
  ...
  virtual double fc(arr& df, arr& Hf, arr& g, arr& Jg, const arr& x) {
    ...
    if (&g) {
      g.clear();
      g.append((sumOfSqr(x)-100.)*(sumOfSqr(x)-100.)); // ||w|| = 1
      g.append(-x); // w > 0
    }
    if (&Jg) {
      Jg.clear();
      Jg=4.*~x*(sumOfSqr(x)-100.);
...
void simpleMotion(){
  ...
  arr refGoal1 = ARR(MP.world.getBodyByName("goal1")->X.pos);
  arr refGoal2 = ARR(MP.world.getBodyByName("goal2")->X.pos);
  TaskCost *c;
  ...
  TaskCost *c2 = MP2.addTask("contact_endeff",tm_contact);
...
//  checkAllGradients(ioc,w,1e-3);
  ...
  arr dual;
  optConstrained(w,dual,ioc,OPT(verbose=0,stopTolerance=1e-3,constrainedMethod=augmentedLag));
  ...
  optConstrained(x2,lambda2,Convert(MPF2),OPT(verbose=0,stopTolerance=1e-7, maxStep=1.));
  MP2.costReport(true);
  cout << lambda2 << endl;
