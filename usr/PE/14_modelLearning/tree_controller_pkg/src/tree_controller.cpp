...
namespace tree_controller_ns {
  ...
  bool TreeControllerClass::init(pr2_mechanism_model::RobotState *robot, ros::NodeHandle &n)
  {
    ...
    if (!chain_.init(robot,"torso_lift_link","r_gripper_tool_frame")) {
      ROS_ERROR("Could not load robot chain");
      return false;
    }
    cout << "chain_.getNumOfJoints(): " << chain_.size() << endl;

    /// Print joint state
//    joint_state_ = robot->getJointState(joint_name);
    jnt_pos_.resize(chain_.size());
    jnt_vel_.resize(chain_.size());
    jnt_efforts_.resize(chain_.size());
    ROS_INFO("Chain sizes: %d",chain_.size());
    chain_.getPositions(jnt_pos_);
    int i;
    for(i=0;i<chain_.size();i++) {
      ROS_INFO("Joint Name %d: %s: %f",i,chain_.getJoint(i)->joint_->name.c_str(), jnt_pos_(i));
    }
    ...
    for (uint i =0;i<controlIdx.d0;i++) {
      ...
      chain_.getJoint(i)->getLimits(low_tmp,high_tmp);
      lowerEffortLimits.append(-0.9*chain_.getJoint(i)->joint_->limits->effort);
      upperEffortLimits.append(0.9*chain_.getJoint(i)->joint_->limits->effort);
    }
  ...
  void TreeControllerClass::update()
  {
    /// pull pos & vel
    chain_.getPositions(jnt_pos_);
    chain_.getVelocities(jnt_vel_);
    chain_.getEfforts(jnt_efforts_);

    /// Convert KDL to ORS
    for (uint i =0;i<controlIdx.d0;i++) {
      q(i) = jnt_pos_(i);
      qd(i) = qd_filt*qd(i)  + (1.-qd_filt)*jnt_vel_.qdot(i);
      measured_effort(i) = jnt_efforts_(i);
    }
    ...
    for (uint i =0;i<controlIdx.d0;i++) {
      ...
      chain_.getJoint(i)->commanded_effort_ = u(i);
      chain_.getJoint(i)->enforceLimits();

      // Additional Safety Check
      if (chain_.getJoint(i)->commanded_effort_>upperEffortLimits(i) || chain_.getJoint(i)->commanded_effort_ < lowerEffortLimits(i)) {
        ROS_ERROR("SAFETY CHECK FAILED! Max u(%d): %f , Min u(%d): %f, u(%d): %f",i,upperEffortLimits(i),i,lowerEffortLimits(i),i,u(i));
        chain_.getJoint(i)->commanded_effort_ = 0.;
      }
    }
