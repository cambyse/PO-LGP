#ifndef NASTYSTOCHASTIC1D_H_
#define NASTYSTOCHASTIC1D_H_

#include <MCTS_Environment/AbstractEnvironment.h>

class NastyStochastic1D : public AbstractEnvironment {
    //----types/classes----//
...
protected:
    ...
    //----methods----//
public:
    NastyStochastic1D(int action_range_,
                 int depth_,
                 double pi_,
                 double rho_,
    ...
    virtual ~NastyStochastic1D() = default;
    virtual observation_reward_pair_t transition(const action_handle_t & action_handle) override {
        ...
        // map position to unit value such that e.g. for action_range*depth=2
        // -2 --> 0.75
        // -1 --> 0.5
        //  0 --> 0.25
        //  1 --> 0
        //  2 --> 1
        double unit = (double)(action_range*depth-state.position-1)/(2*depth*action_range);
        if(unit<0) unit = 1;
        assert(unit>=0 && unit<=1);
        if(deterministic_with_dropout) {
            // give reward in [0,1] in terminal state; reward magnitue scales
            // ANY-linearly with position EXECPT for the positive extrem value;
            // there is a certain drop-out probability (non-zero reward only
            ...
            // give reward of 0 or 1, probability of receiving 1 scales
            // ANY-linearly with position EXECTP for the positive extrem value,
            // starting at 0 and reaching 0.5 at the maximum so that the optimal
            // reward also has highest variance
            if(state.time==depth) {
