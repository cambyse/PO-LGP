    ...
    TEST(InstanceTest, DetachAll) {
        {
            ...
            // create and detach all from nr 8
            EXPECT_EQ(11,create_test_sequence(&ins_set)->next()->detach_all());

            // check use counts and iteration
            for(shared_ptr_t i : ins_set) {
                EXPECT_EQ(2,i.use_count());
                EXPECT_EQ(AbstractInstance::create_invalid(),i->prev());
                EXPECT_EQ(AbstractInstance::create_invalid(),i->next());
            }
        }
        MEMORY_CHECK;
    }
