...
void RAI_Machine::CalcWsgCommand(
  ...
  const InternalState& internal_state =
      context.get_abstract_state<InternalState>(0);
  lcmt_schunk_wsg_command wsgcommand = {0, internal_state.grip, 40.};
...
void RAI_Machine::DoCalcUnrestrictedUpdate(
  ...
  internal_state.world_state.HandleIiwaStatus(internal_state.iiwa_state);
  internal_state.world_state.HandleWsgStatus(internal_state.wsg_state);
  internal_state.world_state.HandleObjectStatus(internal_state.obj_state);
