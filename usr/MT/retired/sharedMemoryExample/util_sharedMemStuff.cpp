...
#else
void mlr::SHM::open(const char *name, uint size, bool directDestroy){
...
//      destroy();
  ...
  p=(byte*)shmat(systemId, NULL, 0); //attach
  if(p==(byte*) -1)
  ...
  uint i;
  if(created){
    if(size<=sizeof(SHMinfoBlock)){
      destroy();
      HALT("need to allocate larger SHM to hold info block!");
    }
    ...
    for(i=0; i<maxShmGuests; i++) info->guestPointers[i]=0;
    info->guestPointers[guestId]=p;
...
void mlr::SHM::close(){
  ...
  shmdt(p); //detach
  created=opened=false; p=NULL; systemId=-1; info=NULL; guestId=-1;
}

void mlr::SHM::destroy(){
...
void mlr::SHM::report(){
  ...
  std::cout
    <<"\n  guestPointers=";
  for(i=0; i<maxShmGuests; i++) std::cout <<' ' <<info->guestPointers[i];
  std::cout
