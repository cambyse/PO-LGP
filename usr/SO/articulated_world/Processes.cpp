...
void FakePerceptionP::step() {
  // robot state
  mlr::Body* agent = geometricState->get_ors().getBodyByName("robot");
  ...
  for (uint i = 0; i < N; i++) {
    mlr::Body* p = geometricState->get_ors().bodies(i);
...
void CognitionP::step() {
  if (worldState->changed) {
    ...
    if (N > 0) {
      mlr::Vector destination = *percepts->objects(::rand() % N);
      mlr::Vector control = destination - robotPos->pos;
