...
UEyeCamera::UEyeCamera(int w, int h, int f): width(w), height(h), fps(f), img(NULL), recthread(NULL), recworker(NULL), recflag(false), setup_flag(false), init_flag(false), open_flag(false), err_flag(false) {
...
void UEyeCamera::camInit() {
  ...
  name.append(STRING("Video_" << camID[cid]));
