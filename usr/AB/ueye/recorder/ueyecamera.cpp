...
UEyeCamera::UEyeCamera(int cid, int w, int h, int f):
  ...
  name = QString("rec/video_%1_.avi").arg(cid);
  image = NULL;
}
...
void UEyeCamera::startRec(QString path) {
  vw = new VideoWriter_x264((path+name).toStdString().c_str(),
                            width,
                            height,
                            fps,
                            20,
...
void UEyeCamera::grab() {
  ...
  //cout << "x";
  //usleep(100); //between 1000 and 5000 should be fine
