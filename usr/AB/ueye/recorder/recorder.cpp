...
void Recorder::newSession() {
  ...
  fname = QString("./rec/session_%1.%2.%3_%4:%5/")
                  .arg(now->tm_year - 100)
                  .arg(now->tm_mon + 1, 2, 10, QChar('0'))
                  .arg(now->tm_mday, 2, 10, QChar('0'))
                  .arg(now->tm_hour, 2, 10, QChar('0'))
                  .arg(now->tm_min, 2, 10, QChar('0'));
  if(mkdir(fname.toStdString().c_str(),
            S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) == 0)
    return;

  for(int fnum = 0;;fnum++) {
    fname = QString("./rec/session_%1.%2.%3_%4:%5_%6/")
                    .arg(now->tm_year + - 100)
                    .arg(now->tm_mon + 1, 2, 10, QChar('0'))
                    .arg(now->tm_mday, 2, 10, QChar('0'))
                    .arg(now->tm_hour, 2, 10, QChar('0'))
