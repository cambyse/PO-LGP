...
and $size(\cdot)$
\item \code{relational/literals.h}: literals are instantiated
symbosl such as $on(a,b)$ and $size(b)=2$; definitions and basic methods
for dealing with literals and sets of literals
\item \code{relational/rules.h}: probabilistic relational rules provide a
...
$\to$ \code{test/relational\_basics}

\item  how to set up a planning scenario \\
(\code{relational/plan.h}, \code{relational/prada.h}) \\
$\to$ \code{test/relational\_plan}

\item how to set up a rule learning run \\
(\code{relational/learn.h}) \\
$\to$ \code{test/relational\_learn}
\end{enumerate}
...
\item Use a very large number of samples \code{numSamples} for PRADA.
