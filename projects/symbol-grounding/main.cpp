...
int main(int argc, char** argv) {
  ...
  sim.shutdownAll();
  sim.loadConfiguration(MT::getParameter<MT::String>("ors_file"));
  sim.startOde();
  sim.startSwift();
  sim.simulate(50);
  ...
  DEBUG_VAR(main, *action);
    ...
    int j; relational::Literal* a;
    for_list(j, a, plan) {
    RMSim::RobotManipulationInterface::performAction(a, &sim, 8);
    }
